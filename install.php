...
if (!$CI->db->table_exists(db_prefix() . 'vapi_calls')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'vapi_calls` (
...
if (!$CI->db->table_exists(db_prefix() . 'vapi_call_events')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'vapi_call_events` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->table_exists(db_prefix() . 'vapi_campaigns')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'vapi_campaigns` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->table_exists(db_prefix() . 'vapi_campaign_leads')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'vapi_campaign_leads` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
...
// Set default webhook token if not exists
if (empty(get_option('vapi_webhook_token'))) {
    $token = bin2hex(random_bytes(32));
    add_option('vapi_webhook_token', $token);
}